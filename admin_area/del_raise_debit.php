...
?> 
        <div class="row">
           ...
           <h2 class="card-title">Raise Debit</h2>
           </div>
           <div class="col-lg-6 col-md-6">
            <a href="index.php?del_debit_sheet" class="btn btn-primary pull-right">Back</a>
           </div>
       </div>
...
if(isset($_POST['submit'])){
    ...
    $today = date("Y-m-d H:i:s");
            ...
            if(!empty($del_partnerArr)){
                for($i = 0; $i < count($del_partnerArr); $i++){
                    if(!empty($del_partnerArr[$i])){
                        ...
                        $insert_debit = "insert into del_debits (delivery_partner_id,del_debit_amt,del_debit_comment,updated_date) 
                        values ('$del_partner','$debit_amt','$debit_comment','$today')";
            ...
            if($run_debit){
                ...
                echo "<script>window.open('index.php?del_debit_sheet','_self')</script>";
            }else{
